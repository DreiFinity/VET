...
const Admin_UserReports = () => {
  ...
  const users = [
    {
      ReportedUsers: "Bangon",
      UserID: "P0000001",
      Reporter: "CARL",
      Reporter_userId: "P0000001",
      Evidence: "FRAUD",
      DateReported: "02/10/2023",
    },
    {
      ReportedUsers: "Bangon",
      UserID: "P0000001",
    ...
    },
    {
      ...
      Reporter_userId: "P0000001",
      Evidence: "FRAUD",
      DateReported: "04/10/2023",
    },
  ];

  const filteredUsers = users.filter(
    (u) =>
      u.ReportedUsers.toLowerCase().includes(search.toLowerCase()) ||
      u.UserID.toLowerCase().includes(search.toLowerCase()) ||
      u.Reporter.toLowerCase().includes(search.toLowerCase()) ||
      u.Reporter_userId.toLowerCase().includes(search.toLowerCase()) ||
      u.Evidence.toLowerCase().includes(search.toLowerCase())
  );
        ...
        <table className="w-full min-w-[800px] border-collapse">
          ...
          <thead>
            <tr className="bg-[#989898] text-white text-left">
              <th className="px-4 py-2 font-regular">Reported Users</th>
              <th className="px-4 py-2 font-regular">User ID</th>
              <th className="px-4 py-2 font-regular">Reporter</th>
              <th className="px-4 py-2 font-regular">UserID</th>
              <th className="px-4 py-2 font-regular">Evidence/Description</th>
              <th className="px-4 py-2 font-regular">Date Reported</th>
          ...
          <tbody>
            {filteredUsers.map((user, index) => (
              ...
              >
                <td className="px-4 py-2">{user.ReportedUsers}</td>
                <td className="px-4 py-2">{user.UserID}</td>
                <td className="px-4 py-2">{user.Reporter}</td>
                <td className="px-4 py-2">{user.Reporter_userId}</td>
                <td className="px-4 py-2">{user.Evidence}</td>
                <td className="px-4 py-2 text-blue-600 underline cursor-pointer">
                  {user.DateReported}
                </td>
                <td className="px-4 py-2">
                  <button
                    onClick={() => alert(`Banned ${user.ReportedUsers}`)}
