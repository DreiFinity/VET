...
const Admin_PetOwners = () => {
  ...
  const fetchUsers = async () => {
    try {
      const res = await axios.get(`${VITE_API_BASE1}/api/users/pet-owners`);
      setUsers(res.data);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      {/* Main Content */}
      <main className="w-full max-w-screen-xl mx-auto p-4 sm:p-6 md:p-8 bg-white shadow-lg rounded-xl border border-gray-300">
        {/* Top Section */}
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-6">
          {/* Search bar */}
          <div className="flex items-center w-full sm:max-w-md bg-gray-200 rounded-full px-4 py-2 shadow-md">
            <input
              type="text"
              placeholder="Search"
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center justify-between w-36 sm:w-40 px-3 py-2 border border-gray-400 rounded-md shadow-sm bg-white text-xs sm:text-sm font-medium"
            >
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <div className="bg-gray-300 px-4 py-3 rounded-2xl text-center flex-1 min-w-[130px] max-w-[160px]">
              <h2 className="text-lg sm:text-xl font-bold">
                {stats.petOwnersCount}
              </h2>
              <p className="text-sm sm:text-base">Pet Owners</p>
            </div>
            <div className="bg-gray-300 px-4 py-3 rounded-2xl text-center flex-1 min-w-[130px] max-w-[160px]">
              <h2 className="text-lg sm:text-xl font-bold">
                {stats.clinicsCount}
              </h2>
              <p className="text-sm sm:text-base">Clinics</p>
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-sm sm:text-base">
            ...
            <tbody>
              {filteredUsers.map((u) => (
                <tr key={u.user_id} className="bg-gray-200 border-b">
